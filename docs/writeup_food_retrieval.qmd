...
**Where it pays off.** This isn't a benchmark for its own sake — the reconciliation it
produces (`food_map`: logged product → USDA `fdc_id`) is what powers the
[knowledge graph's](graph_explorer.qmd) `Meal —EATEN→ Food —HAS_NUTRIENT→ Nutrient`
link. That link lets the graph compute a nutrient the food log *never recorded* — a
day's magnesium, the vitamin D from a specific salmon — by traversing to each food's
full USDA profile. The last-20% of matching accuracy is exactly what makes that
traversal trustworthy.
